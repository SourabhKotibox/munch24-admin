...
import {
  Edit2, Trash2, Search, Plus, Loader2, ImageIcon, Video,
} from "lucide-react";
...
import { useGetSeasonList, useGetEpisodeList, useDeleteEpisode, getImageUrl } from "@/lib/api-client";
...
export default function SeasonsPage() {
  ...
  // Delete a season = delete all episodes in that contentId + season number
  const handleDeleteSeasonEpisodes = async () => {
    ...
    try {
      ...
      const toDelete = allEpisodes.filter(
        (ep: any) =>
          ep.contentId?._id?.toString() === confirmDelete.contentId?.toString() &&
                  ...
                  <TableRow key={season.seasonId} className="border-border hover:bg-muted/30">
                    ...
                    <TableCell>
                      <div className="flex items-center gap-1.5">
                        <button
                          onClick={() => setLocation(`/episodes?contentId=${season.contentId}&season=${season.season}`)}
