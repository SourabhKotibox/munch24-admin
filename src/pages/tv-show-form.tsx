...
export default function TvShowForm() {
  ...
  const [activeTab, setActiveTab] = useState<Tab>("TV Show");
  ...
  const [contentType, setContentType] = useState<"drama" | "series">("series");
  ...
  useEffect(() => {
    ...
    setContentType(content.contentType === "drama" ? "drama" : "series");
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast({ title: isEdit ? "TV Show updated successfully!" : "TV Show created successfully!" });
  ...
  return (
    <div className="space-y-5 pb-6">
      ...
      <div className="flex items-center gap-2 text-sm text-muted-foreground">
        <button onClick={() => setLocation("/tv-shows")} className="hover:text-foreground transition-colors">
          Dashboard
        </button>
        <span>/</span>
        <button onClick={() => setLocation("/tv-shows")} className="hover:text-foreground transition-colors">
        ...
        <span>/</span>
        <span className="text-foreground font-medium">{isEdit ? "Edit TV Show" : "New TV Show"}</span>
      </div>
      ...
      <div className="rounded-xl border border-border bg-card">
        ...
        {activeTab === "TV Show" && (
          <div className="p-6 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <div className="space-y-1.5">
                <Label className="text-foreground text-sm font-medium">Content Type</Label>
            ...
            <div className="space-y-1.5">
              ...
              <Textarea
                placeholder="Full TV show description..."
                value={description}
          ...
          <div className="p-6 space-y-5">
            ...
            <div className="rounded-xl border border-border bg-muted/10 p-5 space-y-5">
              ...
              <div className="space-y-1.5">
                ...
                <div
                  className="min-h-[42px] bg-muted border border-border rounded-lg px-3 py-2 flex flex-wrap gap-1.5 cursor-text"
                  onClick={() => document.getElementById("tag-input-show")?.focus()}
                >
                  {tags.map((kw) => (
      ...
      <div className="flex justify-end">
        ...
        >
          {isSaving ? "Saving..." : isEdit ? "Update TV Show" : "Create TV Show"}
        </Button>
      </div>

      {/* Media Pickers */}
      <MediaPicker open={thumbnailPickerOpen} onClose={() => setThumbnailPickerOpen(false)}
        onSelect={(m) => setThumbnail({ filePath: m.url || m.filePath, preview: m.url || m.filePath })} source="tv-show" accept="image/*" />
      <MediaPicker open={posterPickerOpen} onClose={() => setPosterPickerOpen(false)}
        onSelect={(m) => setPoster({ filePath: m.url || m.filePath, preview: m.url || m.filePath })} source="tv-show" accept="image/*" />
      <MediaPicker open={bannerPickerOpen} onClose={() => setBannerPickerOpen(false)}
        onSelect={(m) => setBanner({ filePath: m.url || m.filePath, preview: m.url || m.filePath })} source="tv-show" accept="image/*" />
      <MediaPicker open={trailerPickerOpen} onClose={() => setTrailerPickerOpen(false)}
        onSelect={(m) => setTrailerFilePath(m.url || m.filePath)} source="tv-show" accept="video/*" />
      <MediaPicker open={seoImagePickerOpen} onClose={() => setSeoImagePickerOpen(false)}
        onSelect={(m) => setSeoImage({ filePath: m.url || m.filePath, preview: m.url || m.filePath })} source="tv-show" accept="image/*" />
    </div>
  );
}
